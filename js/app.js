...
$("#isincountperday").click(function(){
	cleandetail();
	$('#pagecontent').html("Refreshing");
	$.get("utils/embx_ajax.php?pf=isincountperday",function(data){
		$('#pagecontent').html(data);
		$('#pageheader').html("Number of ISINs per Trading Day");
	});
});
   ...
   if (window.File && window.FileReader && window.FileList && window.Blob)
	{
		...
		switch(ftype)
        {
			...
			case 'text/csv'	:
                break;
            default:
                $("#output").html("<b>"+ftype+"</b> XX Unsupported file type!");
				return false
        }

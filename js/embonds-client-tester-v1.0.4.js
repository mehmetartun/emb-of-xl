	...
	function processLine(line, mode) {
		...
		if(mode == 0) {
			...
			if(line[2] === "LOGIN") {
				...
				openSocket($("#server").val(), username, password);
	...
	function openSocket(strHostname, username, password) {
		...
		webSocket.onmessage = function(event){
			...
			*/
			...
			//if (event.data.messages[0].service == "heartbeat") {} else {console.log(event.data);}
	...
	function findBond(bondid){
		var bondname = [];
		for (var i = 0; i < bondList.length; i++){
			if (bondList[i].id == bondid) {
				bondname[0] = bondList[i].bondname;
				bondname[1] = bondList[i].isin;
			}
		}
		return bondname;
	}
		...
		if(isSet(msg.messages)){
			...
			if (isSet(msg.messages[0].service)){
				...
				case "marketview":
					...
					$("#bondrow"+updateBondId).animate({opacity: 0.5},300).delay(100).animate({opacity: 1},300);
					
					
					
					if (msg.messages[0].details.b.w){
						$("#bidprice"+updateBondId).html(numeral(msg.messages[0].details.b.i).format('0.0000'));
						$("#bidsize"+updateBondId).html(numeral(msg.messages[0].details.b.v).format('(0 a)'));
					} else {
						$("#bidprice"+updateBondId).html("");
						$("#bidsize"+updateBondId).html("");
					}
					if (msg.messages[0].details.s.w){
						$("#askprice"+updateBondId).html(numeral(msg.messages[0].details.s.i).format('0.0000'));
						$("#asksize"+updateBondId).html(numeral(msg.messages[0].details.s.v).format('(0 a)'));
					} else {
						$("#askprice"+updateBondId).html("");
						$("#asksize"+updateBondId).html("");
					}
			...
			case "marketview/get":
				if (isSet(msg.response)){
					...
					for (var i = 0; i < msg.response.length; i++){
						...
						if (msg.response[i].b.w){
							$("#bidprice"+updateBondId).html(numeral(msg.response[i].b.i).format('0.0000'));
							$("#bidsize"+updateBondId).html(numeral(msg.response[i].b.v).format('(0 a)'));
						} else {
							$("#bidprice"+updateBondId).html("");
							$("#bidsize"+updateBondId).html("");
						}
						if (msg.response[i].s.w){
							$("#askprice"+updateBondId).html(numeral(msg.response[i].s.i).format('0.0000'));
							$("#asksize"+updateBondId).html(numeral(msg.response[i].s.v).format('(0 a)'));
						} else {
							$("#askprice"+updateBondId).html("");
							$("#asksize"+updateBondId).html("");
						}
					...
					if (isSet(msg.response)){
						...
						for (var i = 0; i < numviews; i++){
							...
							if (msg.response[i].name == "TURKEY"){
								...
								$("#pagecontent").append("<h5>TURKEY<h5>");
							}
						}
				...
				case "authenticate":
					...
					if(isSet(msg.response)) {
						...
						if (lnUserCount === 1) {
							...
							getMarketViews(user.lsLogin);
							subscribeMarketViews(user.lsLogin);
						}
					} else {
	...
	function getMarketViews(username){
		var msg = {
			"action":"marketview/list",
			"details":null
		}
		sendMessage(username, msg);
	}

	function subscribeMarketViews(username){
		var msg = {
	...
	function getMarketViewBonds(username){
		var msg = {
			"action":"marketview/get",
			"details":{
				"ids": marketViewBondIds
			}
		}
		console.log(marketViewBondIds);
		sendMessage(username, msg);
		
	}
	
	function subscribeMarketViewBonds(username){
		var msg = {
			"action":"marketview/subscribe",
			"details":{
				"ids": marketViewBondIds,
	...
	var getWebSocketUrl = function(strHostname) {
		...
		if (strHostname.toLowerCase() === "demo1") {
			return "wss://demo1.embonds.com/EMBonds/socket/1.0";
		}
		...
		if (strHostname.toLowerCase() === "silver1") {
			return "wss://silver1.embonds.com/EMBonds/socket/1.0";
		}

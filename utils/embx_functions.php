	...
	foreach ($lines as $line) {
		...
		if (count($array) >= 8 ){
			...
			if (is_numeric($array[0])){
				$rs = EMBXDB::get()->query("insert into rejects ( logid, fileid, line) values (" . $array[0] . "," . $fileid . ",'" . EMBXDB::get()->escape_string($line) . "')");
				//echo "REJ LOGID - insert into rejects ( fileid, line) values (" . $fileid . ",'" . mysql_escape_string($line) . "') <br/>";
			} else {
				$rs = EMBXDB::get()->query("insert into rejects ( fileid, line) values (" . $fileid . ",'" . EMBXDB::get()->escape_string($line) . "')");
...
function embx_markethistory($isin, $tradingday, $startminute = 420, $endminute = 1080, $interval = 15){
	...
	embx_sql("create table ".$temptablename." select * from orders where date(ordertime) = '".$tradingday."' and isin = '".$isin."'");
	 ...
	 do {
		$mkt = embx_marketsnapshot($isin,$tradingday, $minute, $temptablename);
		//var_dump($mkt);
